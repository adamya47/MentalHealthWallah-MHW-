...
const LoginFunc=async(data)=>{
    try {
        setLoading(true)
        setFault("")
        const user= await axios.post("https://mhw-backend.vercel.app/api/v1/users/login",data,{withCredentials:true})
         if(user){
...
<p className='text-center text-lg '>New User?<span className='text-lg  text-teal-600 hover:underline hover:cursor-pointer ' onClick={()=>navigate("/signup")}>Click here</span></p>
    ...
    <form onSubmit={handleSubmit(LoginFunc)} className="mb-0 mt-6 space-y-4 rounded-lg p-4 shadow-lg sm:p-6 lg:p-8">
